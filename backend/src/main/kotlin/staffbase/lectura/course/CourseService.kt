...
class CourseService(
    ...
    fun deleteCourseByIdForUser(userId: String, courseId: String): Boolean {
        ...
        val course = courseDAO.getById(courseId) ?: return false
        ...
        val wasUserUpdated = userService.updateUser(userId, updatedUser)

        if(wasUserUpdated && !userService.anyUserInCourse(courseId)){
            //courseDAO.delete(courseId)
            val updatedCourse = course.copy(deletedAt = Instant.now())
            val wasCourseUpdated = courseDAO.update(courseId, updatedCourse)
        }
